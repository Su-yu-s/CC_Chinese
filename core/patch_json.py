...
def run_patch(app_dir, progress_cb=None, backup=True) -> dict:
    ...
    self-elevate — the GUI launcher handles UAC up-front (PyInstaller --uac-admin).
    ...
    if not ensure_windowsapps_writable(app_dir):
        return fail(f"无法获取写入权限（WindowsApps 提权失败），请以管理员身份运行后再试。")
    ...
    if not all(src.exists() for src, _ in files):
        return fail("Missing source resource under core/resources/")
    ...
    for src, dst in files:
        if backup and dst.exists():
            files_backed_up.append(str(dst))
            backup_file(dst, app_resources)
        dst.parent.mkdir(parents=True, exist_ok=True)
        if not copy2_best_effort(src, dst, context="json resource"):
            return fail(f"Failed to copy json resource: {src} -> {dst}")
    ...
    patch_desktop_en_us_fallback(app_resources)
    report(60, "写入硬编码界面补丁...")
    hardcoded_labels = patch_hardcoded_ui_fallbacks(app_resources)
